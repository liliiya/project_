...
font = pygame.font.SysFont('Flappy Bird', 60)
...
button_img = load_image('restart.png')
...
class Bird(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        for num in range(1, 4):
            img = load_image(f'bird{num}.png', -1)
...
while run:
    ...
    if len(pipe_group) > 0:
        if bird_group.sprites()[0].rect.left > pipe_group.sprites()[0].rect.left:
            if bird_group.sprites()[0].rect.right < pipe_group.sprites()[0].rect.right and not pass_pipe:
                pass_pipe = True
        if pass_pipe:
    ...
    draw_text(str(score), font, white, int(400), 150)
